...
Single self-contained HTML tool (Sardine-branded). All data and logic are embedded in `index.html`; no build step, no backend.

## ⚠️ Before you publish
This file embeds Sardine's full proprietary list catalog. Keep the repo **private** and run the hosting/access plan past Sardine IT/Security first.
...
```
Create the repo as **Private** in the GitHub UI first (New repository → Private), then run the push above.
...
## If you only need GitHub Pages (NOT recommended here)
Settings → Pages → Source = `main` / root. The site will be **public** — only do this if the data is cleared for public release, which this is not.
